...
export default function ProductsList({ onAddToCartSuccess }) {
  ...
  const { category, subcategory, search: searchParam, tag: tagParam, tags: tagsParam } = useGlobalSearchParams();
    ...
    try {
      ...
      const { data: test, error: testError } = await supabase
        ...
        if (tagParam) {
          ...
          // Use array containment operator for case-insensitive search
          // This assumes tags are stored in lowercase in the database
          query = query.filter('tags', 'cs', `{${normalizedTag}}`);
        }
        
        // Apply multiple tags filter if provided
        if (tagsParam) {
          console.log('Filtering by multiple tags:', tagsParam);
          const tagsList = tagsParam.split(',').map(t => t.toLowerCase().trim());
          
          // For multiple tags, we can use the && operator to ensure all tags are present
        ...
        if (tagParam && validProducts.length > 0) {
          console.log('Client-side filtering by tag:', tagParam);
          const normalizedTag = tagParam.toLowerCase().trim();
          validProducts = validProducts.filter(product => 
            product.tags && 
            Array.isArray(product.tags) && 
            product.tags.some(tag => tag.toLowerCase().includes(normalizedTag))
          );
          console.log('After tag filtering:', validProducts.length, 'products');
        }
        
        // Apply multiple tags filter if provided (client-side fallback)
        if (tagsParam && validProducts.length > 0) {
          console.log('Client-side filtering by multiple tags:', tagsParam);
          const tagsList = tagsParam.split(',').map(t => t.toLowerCase().trim());
          
          validProducts = validProducts.filter(product => 
            product.tags && 
            Array.isArray(product.tags) && 
            tagsList.every(searchTag => 
              product.tags.some(productTag => 
                productTag.toLowerCase().includes(searchTag)
              )
            )
          );
          console.log('After multiple tags filtering:', validProducts.length, 'products');
        }
  ...
  }, [sortOption, selectedCategory, search, category, subcategory, tagParam, tagsParam]);
  ...
  useEffect(() => {
    ...
    console.log('Filter changed - sortOption:', sortOption, 'refreshKey:', refreshKey, 'subcategory:', subcategory, 'tag:', tagParam, 'tags:', tagsParam);
    ...
    fetchProducts(1);
  }, [search, selectedCategory, subcategory, tagParam, tagsParam, sortOption, fetchProducts, refreshKey]);
          ...
          {CATEGORY_OPTIONS.map(cat => (
            <Pressable
              ...
              onPress={() => {
                 ...
                 // Always set the category and reset other filters
                 ...
                 // Navigate to clean URL with only the category parameter
                 // This will reset any other URL parameters like search or tag
                 if (cat === 'All') {
                   // For 'All', remove all parameters
                   router.replace('/(tabs)/shop');
                 } else {
                   // For specific category, set only the category parameter
                   router.replace({
                     pathname: '/(tabs)/shop',
