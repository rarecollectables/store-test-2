...
function MemoCarouselImage({ item, style, onPress }) {
  ...
  if (item.type === 'video') {
    if (Platform.OS === 'web') {
      return (
        <View style={[style, { overflow: 'hidden', backgroundColor: '#000', justifyContent: 'center', alignItems: 'center' }]}>
          <video
            src={item.uri}
            controls
            muted={true}
      ...
      // For mobile, show a placeholder with play icon
      return (
        <View style={[style, { justifyContent: 'center', alignItems: 'center', backgroundColor: '#000' }]}>
          ...
          <Text style={{ color: '#fff', marginTop: 8, textAlign: 'center' }}>Video preview available on web</Text>
        </View>
      );
    }
  }
  ...
  return (
    <Pressable onPress={onPress} accessibilityLabel="Zoom image" accessibilityRole="imagebutton">
      <ExpoImage source={item} style={style} contentFit="contain" transition={300} />
    </Pressable>
  );
}
...
export default function ProductDetail() {
  ...
  const giftBoxOptions = [
    {
      id: 'celebratory',
      name: 'Celebratory Gift Box',
      image: 'https://fhybeyomiivepmlrampr.supabase.co/storage/v1/object/public/utils//packaging.webp',
      description: 'Elegant orange gift box'
    },
    {
      id: 'premium',
      name: 'Premium Gift Box',
      image: 'https://fhybeyomiivepmlrampr.supabase.co/storage/v1/object/public/utils//packaging-2.webp',
      description: 'Luxury cream leather gift box'
    }
  ];

  let images = [];
  if (product) {
    // First, check if there's a video to add to the carousel
    const videoItem = product.video_url ? [
      { 
        type: 'video', 
        uri: product.video_url, 
        poster: product.image_url || product.image,
        id: 'product-video'
      }
    ] : [];
    ...
    if (DATA_IMAGES[product.id]) {
      ...
      // If there are no additional images, just use the main image
      images = [
        ...videoItem,
        ...(product.image_url ? [{ uri: product.image_url }] : [{ uri: product.image }])
      ];
    }
  }

  // Video is now integrated into the carousel
  ...
  // Auto-sliding of images has been disabled as per user request
  // useEffect(() => {
              ...
              >
                ...
                <FlatList
                  ...
                  renderItem={({ item, index }) => (
                    <View style={{ marginHorizontal: 8, paddingBottom: 20 }}>
                      <MemoCarouselImage
                        item={item}
                        style={[
                          styles.image,
                          width < 600
                            ? { width: carouselWidth - 32, height: carouselWidth - 32, resizeMode: 'contain' }
                            : { width: carouselWidth - 32, height: carouselWidth - 32, resizeMode: 'contain' }
                        ]}
                        onPress={() => {
                          setZoomImage(item);
                  ...
                  style={[styles.carousel, { width: carouselWidth, minHeight: carouselWidth }]}
          ...
          <CollapsibleSection title="What You'll Get">
            <View style={{ marginTop: 2 }}>
              ...
              <View style={{ marginTop: 16, marginBottom: 8 }}>
                <Text style={{ fontSize: 16, fontWeight: '600', color: colors.onyxBlack, marginBottom: 10, textAlign: 'center' }}>
                  Choose Your Free Gift Box
                </Text>
                <View style={{ flexDirection: width > 500 ? 'row' : 'column', justifyContent: 'space-between', alignItems: 'center' }}>
                  {giftBoxOptions.map((box) => (
                    <Pressable 
                      key={box.id}
                      style={[{
                        width: width > 500 ? '48%' : '100%',
                        marginBottom: width > 500 ? 0 : 12,
                        borderWidth: 2,
                        borderRadius: 8,
                        padding: 8,
                        alignItems: 'center',
                      ...
                      onPress={() => setSelectedGiftBox(box.id)}
                    >
                      <ExpoImage 
                        ...
                        transition={300}
                      />
                      <Text style={{ fontSize: 15, fontWeight: '600', color: colors.onyxBlack, marginTop: 8 }}>{box.name}</Text>
                      <Text style={{ fontSize: 13, color: colors.platinum, textAlign: 'center', marginTop: 4 }}>{box.description}</Text>
                      
                      {selectedGiftBox === box.id && (
                        <View style={{ position: 'absolute', top: 10, right: 10, backgroundColor: colors.gold, borderRadius: 12, padding: 4 }}>
